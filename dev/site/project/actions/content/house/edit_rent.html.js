...
async function do_house_rent_edit(request, response, cookie, session, resource) {
    ...
    let promise = new Promise(function(resolve, reject){
        ...
        if(HomeHouse.verify_login(cookie, session)){
            ...
            if(hid && 0 < hid){
                do_house_rent_modify(request, response, cookie, session, resource);
            }else{
                do_house_rent_add(request, response, cookie, session, resource).then(function(result){
                    response.type("json");
                    if(result){
                        resolve();
                    }else{
...
async function do_house_rent_add(request, response, cookie, session, resource) {
    log(__filename, "do_edit");
    let self = this;
    let promise = new Promise(function(resolve, reject){
...
async function do_house_rent_modify(request, response, cookie, session, resource) {
    log(__filename, "do_edit");
    let self = this;
    let promise = new Promise(function(resolve, reject){
        let HomeHouse = new T200HomeHouse();
...
global.action.use_post('/content/house/rent/edit', do_house_rent_edit);
