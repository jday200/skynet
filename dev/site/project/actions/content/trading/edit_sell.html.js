...
async function do_trading_sell_edit(request, response, cookie, session, resource) {
    ...
    let promise = new Promise(function(resolve, reject){
        ...
        if(HomeTrading.verify_login(cookie, session)){
            ...
            if(tid && 0 < tid){
                do_trading_sell_modify(request, response, cookie, session, resource);
            }else{
                do_trading_sell_add(request, response, cookie, session, resource).then(function(result){
                    response.type("json");
                    if(result){
                        resolve();
                    }else{
...
async function do_trading_sell_add(request, response, cookie, session, resource) {
...
async function do_trading_sell_modify(request, response, cookie, session, resource) {
    log(__filename, "do_edit");
    let self = this;
    let promise = new Promise(function(resolve, reject){
        let HomeTrading = new T200HomeTrading();
...
global.action.use_post('/content/trading/sell/edit', do_trading_sell_edit);
